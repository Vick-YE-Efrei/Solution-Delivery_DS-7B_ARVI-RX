...
def build_predictor(model_name: str, prompt_path: Path):
    ...
    if model_name == "gemma4_e4b":
        def predict_fn(img):
            return inference.vlm_predict_gemma4(img, prompt_text)
    elif model_name == "medgemma_4b_pt":
        def predict_fn(img):
            return inference.vlm_predict_medgemma(img, prompt_text)
    else:
...
def run(mode: str, db_path: Path, dataset_path: Path | None = None) -> tuple[list[dict], dict]:
    ...
    if mode == "toy":
        ...
        def predict_fn(img):
            from src.inference import vlm_predict_medgemma
            return vlm_predict_medgemma(img, prompt_text)
    elif mode == "improved":
...
def main() -> None:
    ...
    summary = []
    for mode in modes:
        rows, metrics = run(mode, args.db_path, resolve_dataset(args.dataset))
